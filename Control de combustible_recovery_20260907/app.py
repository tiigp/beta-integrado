...
class User(db.Model, UserMixin):
    ...
    @property
    def is_rrhh(self):
        return self.role.strip().lower() == "rrhh"
...
def es_personal_administrativo(colaborador):
    ...
    return bool(re.search(r"\b(administrativ|administracion|contabil|finanz|tesorer|secretari|recepcion|recursos humanos|rrhh|compras|gerencia|comercial|ventas|sistemas|informatica)\w*", normalizado))
...
def inject_dashboard_theme():
    ...
    if notifications_enabled:
        ...
        if rrhh_notifications:
            if current_user.is_supervisor or current_user.is_rrhh:
...
def _build_palanquilla_label_qr(etiqueta: EtiquetaCalidad) -> str:
    lines: list[str] = []
    for label, value in (
...
def _draw_quality_label_pdf(etiqueta: EtiquetaCalidad, output_path: Path) -> None:
    ...
    if etiqueta.tipo_producto == "Palanquillas":
        ...
        if igp_logo.exists():
            pdf.drawImage(str(igp_logo), width / 2 - 14 * mm, height - 29 * mm, 28 * mm, 17 * mm, preserveAspectRatio=True, anchor="c", mask="auto")

        paraguay_logo = QUALITY_LABELS_IMAGES_DIR / "paraguay_logo.png"
        if paraguay_logo.exists():
            pdf.drawImage(str(paraguay_logo), width / 2 - 6 * mm, height - 39 * mm, 12 * mm, 8 * mm, preserveAspectRatio=True, anchor="c", mask="auto")
        ...
        qr.add_data(_build_palanquilla_label_qr(etiqueta))
        qr.make(fit=True)
        qr_image = qr.make_image(fill_color="black", back_color="white")
        pdf.drawImage(ImageReader(qr_image.get_image()), width - 38 * mm, height - 77 * mm, width=28 * mm, height=28 * mm, preserveAspectRatio=True, mask="auto")
        pdf.save()
        return
    ...
    field("Colada", etiqueta.colada, margin, top - 28 * mm)
    ...
    if igp_logo.exists():
        ...
        for pixel in watermark.getdata():
            ...
            alpha = max(0, min(42, int((255 - luminance) * 0.28)))
...
def _draw_quality_certificate_pdf(certificado: CertificadoCalidad, output_path: Path) -> None:
    ...
    watermark = QUALITY_LABELS_IMAGES_DIR / "marcadeagua.png"
    if watermark.exists():
        watermark_image = PILImage.open(watermark).convert("RGBA")
        watermark_pixels = []
        for red_value, green_value, blue_value, _ in watermark_image.getdata():
            luminance = (red_value + green_value + blue_value) / 3
            alpha = max(0, min(58, int((255 - luminance) * 0.32)))
            watermark_pixels.append((red_value, green_value, blue_value, alpha))
    ...
    logo = QUALITY_LABELS_IMAGES_DIR / "igp_logo.png.jpg"
    if logo.exists():
        pdf.drawImage(str(logo), left, page_height - 22 * mm, 38 * mm, 19 * mm, preserveAspectRatio=True, anchor="c", mask="auto")
    pdf.setFillColor(navy)
    ...
    pdf.line(left, page_height - 22 * mm, right, page_height - 22 * mm)
    ...
    def force_values(stress_value: str) -> str:
        ...
        for diameter in selected_measures:
            ...
            values.append(f"Ø{diameter_mm:g}: {force_newtons:,.0f} N".replace(",", "."))
        return f"{stress_value} MPa" + (f"  |  {'; '.join(values)}" if values else "")

    gap = 4 * mm
    box_width = (right - left - gap) / 2
    rows = [
        (("N° de certificado", certificado.numero_certificado), ("Carbono (%)", certificado.carbono)),
    ...
    firma_nombre = get_config_value("calidad_firma_nombre", "Ing. Rubén Miranda")
...
def quality_dashboard():
    if request.method == "POST":
        ...
        if not calidad or not colada:
            flash("Debe completar la calidad y la colada antes de guardar.", "error")
            return redirect(url_for("quality_dashboard"))
...
def reporte_cierre_turno_qality():
    ...
    if request.method == "POST":
        try:
            fecha_desde = datetime.strptime(request.form.get("fecha_desde", ""), "%Y-%m-%dT%H:%M")
            fecha_hasta = datetime.strptime(request.form.get("fecha_hasta", ""), "%Y-%m-%dT%H:%M")
        except ValueError:
        ...
        if fecha_desde > fecha_hasta:
            ...
            pdf = _build_quality_shift_pdf(etiquetas, resumen, fecha_desde, fecha_hasta, f"{area} · {turno}")
...
def descargar_etiqueta_qality(etiqueta_id: int):
    ...
    pdf_path = QUALITY_LABELS_DIR / f"etiqueta_{etiqueta.id:07d}.pdf"
    _draw_quality_label_pdf(etiqueta, pdf_path)
    return send_file(pdf_path, mimetype="application/pdf", as_attachment=not request.args.get("preview"), download_name=pdf_path.name)
...
def vista_previa_etiqueta_qality(etiqueta_id: int):
    ...
    if not pdf_path.exists():
        _draw_quality_label_pdf(etiqueta, pdf_path)
    return render_template("vista_previa_etiqueta.html", etiqueta=etiqueta)
...
def editar_etiqueta_qality(etiqueta_id: int):
    ...
    if request.method == "POST":
        ...
        if tipo_producto == "Palanquillas":
            ...
            etiqueta.hornero = etiqueta.supervisor = etiqueta.operador_ccm = ""
...
def importar_etiquetas_csv_qality():
    ...
    try:
        ...
        aliases = {
            ...
            "lote": {"lote"}, "cantidad": {"cantidad"}, "fecha": {"fecha"}, "observaciones": {"observacion", "observaciones"},
        }
        fields = {}
        ...
        for row in reader:
            ...
            etiqueta.tipo_producto = {"ap500s": "AP500S", "varillaslisas": "VarillasLisas", "palanquillas": "Palanquillas"}.get(raw_type, str(value("tipo_producto") or "").strip())
            etiqueta.calidad = str(value("calidad") or "").strip()
            etiqueta.medida = str(value("medida") or "").strip()
            etiqueta.longitud = str(value("longitud") or "").strip()
            etiqueta.peso = str(value("peso") or "").strip()
        ...
        db.session.rollback()
        flash("No se pudo importar el CSV. Use UTF-8 y columnas tipo_producto, calidad y colada.", "error")
    return redirect(url_for("consultar_etiquetas_qality"))
...
def control_calidad():
    ...
    return render_template(
        ...
        warning=warning,
        filtro=query,
        calidad_firma_nombre=get_config_value("calidad_firma_nombre", "Ing. Rubén Miranda"),
...
def historial_despacho_acero():
    ...
    return render_template("historial_despacho_acero.html", despachos=despachos, cliente=cliente)
...
def rrhh():
    ...
    return render_template("rrhh.html", notifications=notifications, vacation_notifications=vacation_notifications)
...
def rrhh_vacaciones():
    if request.method == "POST":
        ...
        try:
            ...
            dias = 0
        if not colaborador or not colaborador.activo or not colaborador.fecha_ingreso or dias <= 0 or not inicio:
            flash("Seleccione un colaborador activo, una fecha de inicio y una cantidad válida de días.", "error")
        else:
    ...
    return render_template("rrhh_vacaciones.html", colaboradores=colaboradores, agendas=agendas, hoy=datetime.utcnow().date())
...
def generar_recordatorios_rrhh():
    ...
    for colaborador in Colaborador.query.filter_by(activo=True).all():
        ...
        fin_prueba = colaborador.fecha_ingreso + timedelta(days=90)
        dias_prueba = (fin_prueba - hoy).days
        avisos = []
        if dias_aniversario in {0, 15}:
            momento = "hoy" if dias_aniversario == 0 else "en 15 días"
            avisos.append((f"Evaluación anual: {colaborador.nombre}", f"RRHH-FOR-007 para {colaborador.nombre}; cumple un año {momento}.", "periodica"))
        if dias_prueba in {0, 15}:
            momento = "hoy" if dias_prueba == 0 else "en 15 días"
            avisos.append((f"Evaluación de prueba: {colaborador.nombre}", f"RRHH-FOR-008 para {colaborador.nombre}; finaliza el período de prueba {momento}.", "prueba"))
...
def calcular_estado_colaborador(colaborador):
    ...
    limite = colaborador.fecha_ingreso + timedelta(days=90)
...
def rrhh_evaluaciones():
    ...
    for candidato in candidatos:
        ...
        if es_personal_administrativo(candidato):
            candidato_formulario = "administrativa"
        else:
            candidato_formulario = "prueba" if candidato.estado.lower() == "prueba" else "periodica"
    ...
    if request.method == "POST":
        ...
        if colaborador:
            colaborador.estado = calcular_estado_colaborador(colaborador)
        es_formulario_valido = not form_config.get("administrativa") or es_personal_administrativo(colaborador)
        ...
        if not colaborador or not colaborador.activo or estado_invalido or not es_formulario_valido:
            ...
            return redirect(url_for("rrhh_evaluaciones", legajo=colaborador.legajo, formulario=formulario, popup="1" if popup else None))
...
def editar_documento_rrhh(documento_id: int):
    ...
    if extension == ".xlsx":
        ...
        workbook.close()
        return render_template("rrhh_documento_editar.html", documento=documento, tipo_editor="xlsx", rows=rows, hoja=sheet.title, filas_mostradas=max_rows, filas_totales=sheet.max_row, columnas_mostradas=max_columns, columnas_totales=sheet.max_column)
    if extension == ".docx":
        word = WordDocument(str(target))
        contenido = "\n".join(paragraph.text for paragraph in word.paragraphs)
    else:
        contenido = target.read_text(encoding="utf-8")
    return render_template("rrhh_documento_editar.html", documento=documento, tipo_editor="text", contenido=contenido)
